...
export function AssignBranchManagerDialog({
  ...
  async function onSubmit(values: FormValues) {
    try {
      ...
      toast.success("Đã đổi quản lý chi nhánh");
      onOpenChange(false);
      onSuccess?.();
    } catch (error) {
      toast.error(getApiErrorMessage(error));
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Đổi quản lý chi nhánh</DialogTitle>
          <DialogDescription>
            Chọn nhân viên STAFF đang hoạt động trong chi nhánh. BE sẽ thăng
            người này thành BRANCH_MANAGER và hạ quản lý hiện tại về STAFF.
          </DialogDescription>
        </DialogHeader>
