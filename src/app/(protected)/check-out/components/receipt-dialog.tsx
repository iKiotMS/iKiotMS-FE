    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] flex flex-col p-6">
        ...
        <div className="flex-1 overflow-y-auto py-4 px-2 my-2 border border-dashed rounded bg-slate-50/50 dark:bg-slate-900/30">
          <div ref={receiptRef} className="text-sm space-y-4 font-sans text-foreground">
            {/* Header info */}
            <div className="text-center space-y-1">
              <h2 className="text-lg font-bold tracking-tight text-foreground uppercase">HÓA ĐƠN BÁN HÀNG</h2>
              <p className="text-xs text-muted-foreground font-mono">{order.orderCode}</p>
            ...
            <div className="grid grid-cols-2 gap-y-1 text-xs">
              ...
              {order.customer?.phone && (
                <>
                  ...
                  <span className="text-right font-mono">{order.customer.phone}</span>
                </>
              )}
            </div>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-dashed text-muted-foreground">
                  ...
                  <th className="py-2 text-right font-semibold w-24">Đơn giá</th>
                  <th className="py-2 text-right font-semibold w-24">Thành tiền</th>
                </tr>
              </thead>
              <tbody>
                {order.items.map((item, index) => {
                  const lineTotal = item.quantity * item.unitPrice - item.discountAmount * item.quantity;
                  return (
                    <tr key={index} className="border-b border-dashed border-muted/30">
                      <td className="py-2">
                      ...
                      <td className="py-2 text-right tabular-nums">{formatVND(item.unitPrice)}</td>
                      <td className="py-2 text-right font-medium tabular-nums">{formatVND(lineTotal)}</td>
                    </tr>
                  );
                })}
              </tbody>
            ...
            <div className="space-y-1.5 text-xs">
              ...
              {order.items.reduce((acc, item) => acc + item.discountAmount * item.quantity, 0) > 0 && (
              ...
              <div className="flex justify-between font-bold text-sm border-t border-dashed pt-2 mt-2">
                ...
                <span className="tabular-nums text-primary">{formatVND(order.grandTotal)}</span>
              </div>

              <div className="flex justify-between pt-1">
                <span className="text-muted-foreground">Khách thanh toán ({paymentMethodLabel[order.paymentMethod] || order.paymentMethod}):</span>
              ...
              {order.note && (
                <div className="pt-2 border-t border-dashed mt-2">
                  <div className="text-muted-foreground text-[11px]">Ghi chú:</div>
